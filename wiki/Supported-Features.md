...
| `lotus-idea` proposal-intake realization | Executable initial adviser-review realization; not yet a supported product feature | `POST /advisory/proposals/idea-intake`; `GET /advisory/proposals/idea-intake/{intake_id}/realization`; `contracts/idea-proposal-intake/lotus-advise-idea-proposal-intake.v1.json` | Proves source-safe handoff, deterministic portfolio-scoped review-work identity, one durable `PENDING_ADVISER_REVIEW` work item, append-only initial Advise-owned outcomes, restart-safe replay, business-identity conflict detection, and fail-closed scoped read. It does not create or link proposal records, publish later/terminal review outcomes, run suitability, grant approval, create orders, authorize client publication, certify a data product, or infer downstream success. |

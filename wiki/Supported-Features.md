...
| Proposal review benchmark/mandate-limit evidence | Typed unavailable posture only | `ProposalResult.proposal_review_evidence` in simulation and lifecycle version responses | Requested selectors are retained as request context; effective benchmark assignment and mandate-limit observations remain `UNAVAILABLE` until an authoritative producer contract is supplied. Advise does not infer them from generic rules or calculate them locally. |

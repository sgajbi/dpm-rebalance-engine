...
| Proposal review benchmark/mandate-limit evidence | Typed unavailable posture only | `ProposalResult.proposal_review_evidence` in simulation and lifecycle version responses | Requested selectors are retained as request context. Core has an existing benchmark-assignment route, but Advise does not yet consume or map it; no mandate-limit observation producer is mapped. Effective evidence remains `UNAVAILABLE` until source-owned evidence is mapped. Advise does not infer it from generic rules or calculate it locally. |
...
| `lotus-idea` proposal-intake realization | Executable adviser-review, proposal-linkage, and source-owned terminal realization; not yet a supported cross-repository product feature | `POST /advisory/proposals/idea-intake`; scoped reads by intake or Idea-owned conversion intent; `POST /advisory/proposals/idea-intake/{intake_id}/realization/proposal-reconciliation`; `contracts/idea-proposal-intake/lotus-advise-idea-proposal-intake.v1.json` | Proves source-safe handoff, deterministic portfolio-scoped review-work identity, append-only Advise outcomes, read-only lost-response recovery, explicit one-to-one linkage to an existing same-portfolio proposal, compare-and-set progression, restart-safe replay, terminal mapping from authoritative proposal state, and fail-closed scoped access. It does not auto-create proposals, run suitability, grant approval, create or independently prove orders/fills/settlement, authorize client publication, bind production IdP claims, certify Idea consumer reconciliation, or infer business success from transport. |
...
| AdvisoryPolicyEvaluationRecord:v1 | Supported for advisor/compliance policy evidence | `/advisory/policy-packs*`; `/advisory/proposals/{proposal_id}/versions/{proposal_version_id}/policy-evaluations`; `/advisory/policy-evaluations/*`; `GET /platform/capabilities`; `contracts/domain-data-products/`; `contracts/trust-telemetry/`; platform mesh SLO/access/evidence policy | `AdvisoryPolicyEvaluationRecord:v1` is active as source-owned policy evaluation evidence. It covers policy-pack catalog/activation, source-readiness evidence, finalized evaluation records, replay, review queues, sign-off source packages, workflow/sign-off posture, source-owned as-of and trusted scope receipt identity for downstream `lotus-idea` reconciliation, signed-off report-package lineage, bounded AI evidence lineage, Gateway/Workbench consumption, live-suite proof, and freshness-gated trust telemetry. Runtime policy catalog, audit, idempotency, and evaluation state are backed by repository ports and the `policy_packs` Postgres migration namespace. Workflow receipt metadata exposes legal-entity, booking-center, proposal, version, portfolio, and source-safe tenant/service/correlation/trace hashes; it does not bind production IdP claims. Completed approval/waiver authority, completed sign-off authority, client-ready policy publication, and external client communication remain gated; RFC-0028 governs bank-demo/RFP proof through supported claims. |
...
| Proposal decision vocabulary contract | Supported | `docs/standards/proposal-decision-vocabulary.v1.json`; `make proposal-decision-vocabulary-gate` | Advise is the vocabulary authority. Workflow-gate pairings are checked against the runtime gate inverse and insufficient-evidence next actions against evidence-gap branches. The legacy top-level status projection is an explicit reviewed compatibility declaration; approval-requirement and gate-reason evidence remain separate fields. |
...
| API vocabulary and no-alias governance | Supported | `make api-vocabulary-gate`, `make proposal-decision-vocabulary-gate`, and `make no-alias-gate`. |
...
| License/IP release evidence | Supported | `make license-ip-gate`, isolated runtime/development requirements-installed evidence generation with pinned pip/setuptools bootstrap tooling, `docs/standards/license-ip-policy.v1.json`, `docs/standards/license-ip-inventory.v1.json`, and `NOTICE.md`. |

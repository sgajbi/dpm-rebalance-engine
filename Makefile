...
check: lint typecheck openapi-gate no-alias-gate api-vocabulary-gate proposal-decision-vocabulary-gate domain-data-products-gate trust-telemetry-freshness-gate advisory-data-lifecycle-gate durable-state-recovery-gate external-adapter-contracts advisory-copilot-evaluation-gate advisory-copilot-safety-gate docs-source-reference-gate wiki-quality-gate slo-capacity-gate migration-rollout-contract-gate quality-baseline-check quality-trend-gate dead-code-gate duplicate-code-gate unused-dependency-gate oversized-code-gate bandit-severity-regression-gate dependency-lock-gate license-ip-gate release-image-provenance-gate test

ci: verify-dependencies lint typecheck openapi-gate no-alias-gate api-vocabulary-gate proposal-decision-vocabulary-gate domain-data-products-gate trust-telemetry-freshness-gate advisory-data-lifecycle-gate durable-state-recovery-gate advisory-copilot-evaluation-gate advisory-copilot-safety-gate docs-source-reference-gate wiki-quality-gate slo-capacity-gate migration-rollout-contract-gate quality-baseline-check quality-trend-gate dead-code-gate duplicate-code-gate unused-dependency-gate oversized-code-gate migration-smoke security-audit dependency-lock-gate license-ip-gate release-image-provenance-gate coverage-combined changed-coverage-gate docker-build postgres-runtime-contracts-local production-profile-guardrail-negatives-local
...
ci-local: verify-dependencies lint typecheck openapi-gate no-alias-gate api-vocabulary-gate proposal-decision-vocabulary-gate domain-data-products-gate trust-telemetry-freshness-gate advisory-data-lifecycle-gate durable-state-recovery-gate docs-source-reference-gate wiki-quality-gate slo-capacity-gate migration-rollout-contract-gate quality-baseline-check quality-trend-gate dead-code-gate duplicate-code-gate unused-dependency-gate oversized-code-gate migration-smoke security-audit dependency-lock-gate license-ip-gate release-image-provenance-gate coverage-combined changed-coverage-gate
...
check-all: lint typecheck proposal-decision-vocabulary-gate quality-trend-gate dead-code-gate duplicate-code-gate unused-dependency-gate oversized-code-gate wiki-quality-gate test-all
...
wiki-quality-gate:
	python -m scripts.wiki_quality_gate --base-ref "$(QUALITY_BASE_REF)" --head-ref "$(QUALITY_HEAD_REF)"
...
quality-trend-gate:
	python -m scripts.quality_trend_gate --base-ref "$(QUALITY_BASE_REF)" --head-ref "$(QUALITY_HEAD_REF)" --policy quality/quality-trend-policy.v1.json --output output/quality-trend-gate.json
...
oversized-code-gate:
	python -m scripts.oversized_code_gate --policy quality/oversized-code-policy.v1.json --output output/oversized-code-gate.json
